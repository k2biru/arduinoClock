...
#define DS3231_TEMP_MSB  0x11         // alamat 
...
float getTemp3231Celcius()
{
  ...
  uint8_t cacah = (Wire.read() >> 6) * 25;             // jawaban kedua berupa 1/4 fraksi (1/4 ; 2/4 ; 3/4 ; 4/4)
  float derajat = (float)riil;
  derajat += (float)cacah / ((derajat < 0) ? -100.0f : 100.0f) ; // bila derajat<0 maka dibagi -100.0f bila tidak dibagi 100.0f (baca datasheet ds3231 #bagian  negatif)
  return derajat;
}
