...
void handleHome()
{
  ...
  text += "<p>Setting Wifi</p><form name =\"fs\"method='POST' action='/update' target='_blank'>";
  ...
  if (n == 0) text += " !! Tidak ada Jaringan Tersedia !! <br>";
    ...
    for (byte i  = 0; i < n; i++)
    {
      text += "<tr><th><a href=\"#\" onclick=\"document.fs.newSSID.value='"; text += WiFi.SSID(i); text += "';return false;\">"; text += WiFi.SSID(i); text += "</a></th><th>"; text += WiFi.RSSI(i); text += "</th><th>"; text += (WiFi.encryptionType(i) == ENC_TYPE_NONE) ? " " : "*" ;
      text += "</th></tr>";
      delay(10);
    }
    text += "</table><br>===============================<br>";
  }
  text += "<form method=\"POST\" action=\"/update\" target=\"_blank\">";
  text += "  Tanggal:<br>";
  text += "<input type=\"date\" name=\"date\" id=\"myDate\"/>";
  text += "  <br>Jam:<br>";
  text += "<input type=\"time\" name=\"time\" id=\"myTime\"/>";
  text += "  <input type=\"submit\" value=\"Perbaruhi\">";
  text += "</form>";
  ...
  text += "<form method=\"POST\" action=\"/update\" target=\"_blank\">";
  text += " Kecerahan Baru<br>Kecerahan hanya berlaku saat jam 4 pagi s/d 22 malam<br>";
  text += "<input type=\"range\" name=\"brighness\" value=\"500\" max=\"1024\">"; 
  text += "<input type=\"submit\" value=\"Perbaruhi\">";
  text += "</form>";
...
void handleSentData() {
  ...
  if (server.arg("brighness") != "")
  {
    Serial.print("#B");
    Serial.println(server.arg("brighness").toInt());
  }
  if (server.arg("date") != "" || server.arg("time") != "")
  {
    char temp[14], temp1[2];
    byte i;
    ...
    sprintf(temp, "#C");
    if (jam < 10)sprintf(temp, "%s0%d",temp, jam); else sprintf(temp, "%s%d",temp, jam);
    if (menit < 10)sprintf(temp, "%s0%d",temp, menit); else sprintf(temp, "%s%d",temp, menit);
    sprintf(temp, "%s01",temp);
    Serial.println(temp);
    sprintf(temp, "#D");
    if (tg < 10)sprintf(temp, "%s0%d",temp, tg); else sprintf(temp, "%s%d",temp, tg);
    if (bl < 10)sprintf(temp, "%s0%d",temp, bl); else sprintf(temp, "%s%d",temp, bl);
    if (th < 10)sprintf(temp, "%s0%d",temp, th); else sprintf(temp, "%s%d",temp, th);
    delay(2000);
    Serial.println(temp);
  ...
  {
    server.arg("newSSID").toCharArray(data.ssid, 20);
    if (server.arg("newPass") != "")server.arg("newPass").toCharArray(data.pass, 20);
    else sprintf(data.pass, "");
    EEPROM.begin(512);
